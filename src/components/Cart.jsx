      ...
      {sortedCartLists.map((item) => (
        <div class="flex flex-wrap gap-x-4 overflow-hidden rounded-lg border sm:gap-y-4 lg:gap-6 bg-gray-200">
          ...
          <div class="flex flex-1 flex-col justify-between py-4 ">
            <div>
              <a href="#" class="mb-1 inline-block text-lg font-bold text-gray-800 transition duration-100 hover:text-gray-500 lg:text-xl">{item.title}</a>
            ...
            <div>
              <span class="mb-1 block font-bold text-gray-800 md:text-lg">{item.prevPrice}</span>
          ...
          <div class="flex w-full justify-between border-t p-4 sm:w-auto sm:border-none sm:pl-0 lg:p-6 lg:pl-0 ">
            <div class="flex flex-col items-start gap-2">
              ...
              <button onClick={() => handleDelete(item.id)}class="select-none text-sm font-semibold text-indigo-500 transition duration-100 hover:text-indigo-600 active:text-indigo-700">Delete</button>
            </div>
  
            <div class="ml-4 pt-3 md:ml-8 md:pt-2 lg:ml-16">
              <span class="block font-bold text-gray-800 md:text-lg">{item.price}</span>
            </div>
          </div>
        </div>
      ))}
